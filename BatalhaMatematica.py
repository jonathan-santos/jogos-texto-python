...
print(
    "Leptude por algum motivo entra em batalha com o sapo mago professor de matemática interdimensional Jaque, para vencer dele você precisa acertar suas perguntas de matemática ou então ele vai te matar de tédio")
...
while vidaLeptude > 0 and vidaJaque > 0:
    if vezJaque:
        ...
        while con:
            try:
                resposta = int(input("Jaque te ataca com a pergunta: quanto é " + perguntaJaque + "? "))
            except:
                print("escreva certo para o jaque enteder você")
            else:
                con -= 1
        print("A resposta da pergunta era", eval(perguntaJaque))
        if resposta == eval(perguntaJaque):
            print("Acertou mizerávi")
        else:
            vidaLeptude -= 1
            print("Errou mizerávi, agora você está com", vidaLeptude, "pontos de vida")
    else:
        a = str(input("Agora Leptude, ataque jaque com uma conta de matemática: ")).split()
        resposta = eval(''.join(a))
...
if (vidaJaque > 0):
    ...
    print(
        "Jaque perdeu a batalha, então você começa a zuar ele, ele fica nervoso e volta a sua dimensão para ter reforço de matemática")
